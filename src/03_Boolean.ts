...
//4)
function booleanCondition(b1:boolean , b2:boolean):boolean{
    if(b1 = b2 = false){
        return false
    }else{
        return true
...
function checkNumber(num : number):boolean |number{
    if(num > 0){
        return true
    }else if( num < 0){
        return false
...
function checkYearLeap(year : number):boolean{
    if(year % 400 ==0){
        return true
    }else{
        return false
...
function checkStrNumber(str:string):boolean{
    let p4numcheck = /\d/
